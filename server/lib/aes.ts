/**
 * AES (Rijndael) implementation for the Express backend.
 * Mirrors src/lib/aes.ts; adds decryptBlockWithSteps for step-by-step API.
 * Educational only — not for production cryptography.
 */
...
export function encryptBlockWithSteps(block: Uint8Array, key: Uint8Array): CipherRun {
  ...
  steps.push({
    kind: 'init', round: 0, totalRounds: Nr,
    title: 'State Initialization',
    description: 'The 16-byte input block is mapped column-by-column into a 4×4 matrix of bytes called the State. Byte 0 → row 0 col 0, byte 1 → row 1 col 0, and so on.',
  ...
  steps.push({
    ...
    description: 'Each byte of the State is XORed with the corresponding byte of round key 0 (the original key). This is the only operation before the main rounds begin.',
    before: state, after: next, roundKey: roundKeys[0],
  });
  state = next;
  ...
  for (let r = 1; r <= Nr; r++) {
    ...
    next = subBytes(state);
    steps.push({
      kind: 'subBytes', round: r, totalRounds: Nr,
      title: `Round ${r} — SubBytes`,
      description: 'Each byte is replaced via the non-linear S-box: b → S[b]. Provides confusion and breaks linear relationships.',
    ...
    steps.push({
      kind: 'shiftRows', round: r, totalRounds: Nr,
      title: `Round ${r} — ShiftRows`,
      description: 'Row 0 unchanged. Rows 1, 2, 3 are cyclically shifted left by 1, 2, 3 positions respectively. Provides diffusion across columns.',
      before: state, after: next, isFinalRound: isFinal,
    });
    state = next;
    ...
    if (!isFinal) {
      ...
      steps.push({
        kind: 'mixColumns', round: r, totalRounds: Nr,
        title: `Round ${r} — MixColumns`,
        description: 'Each column is treated as a polynomial over GF(2⁸) and multiplied by a fixed matrix. Fully mixes bytes within each column for diffusion.',
    ...
    steps.push({
      ...
      description: isFinal
        ? 'Final round: XOR with the last round key. MixColumns is skipped here — this asymmetry makes the cipher invertible without extra overhead.'
  ...
  steps.push({
    kind: 'output', round: Nr, totalRounds: Nr,
...
export function decryptBlockWithSteps(block: Uint8Array, key: Uint8Array): DecipherRun {
  ...
  steps.push({
    ...
    description: 'The 16-byte ciphertext block is loaded into the 4×4 State matrix. Decryption will peel back each transformation in reverse.',
    before: initial, after: initial,
  });
  ...
  steps.push({
    ...
    description: `XOR the State with the last round key (K${Nr}). Decryption starts here and works backwards through the key schedule.`,
  ...
  for (let r = Nr - 1; r >= 1; r--) {
    ...
    steps.push({
      ...
      description: 'Inverse of ShiftRows: rows 1, 2, 3 are cyclically shifted right by 1, 2, 3 positions respectively.',
      before: state, after: next, isFinalRound: isFinal,
    });
    state = next;
    ...
    steps.push({
      ...
      description: 'Each byte is replaced via the inverse S-box: b → S⁻¹[b]. Reverses the non-linear substitution applied during encryption.',
      before: state, after: next, isFinalRound: isFinal,
    });
    state = next;
    ...
    steps.push({
      ...
      description: `XOR with round key K${r}. Since XOR is its own inverse, this undoes the AddRoundKey from the encryption round ${r}.`,
      before: state, after: next, roundKey: roundKeys[r], isFinalRound: isFinal,
    });
    state = next;
    ...
    steps.push({
      ...
      description: 'Each column is multiplied by the inverse MixColumns matrix over GF(2⁸). Reverses the diffusion step from encryption.',
      before: state, after: next, isFinalRound: isFinal,
    });
    state = next;
  }
  ...
  steps.push({
    ...
    description: 'Shift rows right to undo the first encryption round\'s ShiftRows.',
  ...
  steps.push({
    ...
    description: 'Apply the inverse S-box to each byte.',
    before: state, after: next, isFinalRound: true,
  });
  state = next;
  ...
  steps.push({
    ...
    description: 'XOR with the original key (K0). This is the last step — the State now holds the original plaintext.',
...
export function encryptMessage(
  text: string,
  key: Uint8Array,
): { ciphertext: Uint8Array; firstRun: CipherRun; blocks: Uint8Array[] } {
...
export function decryptMessage(ciphertext: Uint8Array, key: Uint8Array): string {
  ...
  for (let i = 0; i < ciphertext.length; i += 16) {
    blocks.push(decryptBlockWithSteps(ciphertext.slice(i, i + 16), key).plaintext);
  }
